...
# read in raw data
cat <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\Blinded category.csv")
pwa <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\PWA.csv")
pwv <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\PWV.csv")
cIMT <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\cIMT.csv")
BrachD <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\BrachD.csv")

# clean up extra rows
cat <- cat[!is.na(cat$StudyID),] 
...
pwv_mean <- aggregate(pwv[,c("MeanHeartRate","MeanPulseWaveVelocity")],by=list(pwv$StudyID),mean,na.rm=TRUE)
colnames(pwv_mean) <- c("StudyID","MeanHeartRate","MeanPulseWaveVelocity")
pwv_mean <- pwv_mean[!is.na(pwv_mean$MeanHeartRate),]

pwa_mean <- aggregate(pwa[,c("C_Ap","C_Ph","C_Agph")],by=list(pwa$StudyID),mean,na.rm=TRUE)
colnames(pwa_mean) <- c("StudyID","C_Ap","C_Ph","C_Agph")
pwa_mean <- pwa_mean[!is.na(pwa_mean$C_Ph),]

bd_mean <- aggregate(BrachD[,"BA.Distensibility....mmHg."],by=list(BrachD$StudyID),mean,na.rm=TRUE)
...
# results that don't match data collection sheet
check <- alldata[is.na(alldata$AStatus),]
write.csv(check,"S:\\Shared Projects\\Laura\\BDC\\Projects\\CACTI\\CACTI oral microbiome\\Blinded CSOC report #2\\Data\\check.csv")

# currently there is one ppt with Group.Status="N" - take them out for now
alldata$Group.Status <- alldata$AStatus
